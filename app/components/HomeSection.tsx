...
const translations = {
  id: {
    ...
    iam: "Saya",
    role: "Pengembang Web",
    desc: "Seorang pengembang web yang senang membangun produk digital yang berdampak. Berfokus pada kode yang bersih, interaksi yang mulus, dan pengalaman pengguna yang bermakna.",
    download: "Unduh CV",
    talk: "Mari Bicara",
  },
  eng: {
    ...
    iam: "I am",
    role: "Web Developer",
    desc: "A web developer who enjoys building impactful digital products. Focusing on clean code, smooth interactions, and meaningful user experiences.",
  ...
  jpy: {
    ...
    iam: "私は",
    role: "ウェブ開発者",
    desc: "インパクトのあるデジタル製品を作るのが好きなウェブ開発者です。クリーンなコード、スムーズなインタラクション、そして意味のあるユーザーエクスペリエンスに焦点を当てています。",
    download: "履歴書をダウンロード",
    talk: "話しましょう",
  },
};
...
export default function HomeSection() {
  ...
  // --- KABEL KOMUNIKASI BAHASA ---
  useEffect(() => {
    const handleLangChange = (e: any) => {
      // e.detail berisi "id", "eng", atau "jpy" dari Navbar
      setLang(e.detail);
    };
      ...
      <div className={`relative z-20 max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 gap-10 items-center transition-all duration-1000 ${show ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}>
        <div className={`order-2 md:order-1 transition-all duration-1000 delay-300 ${show ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}>
          ...
          <div className="flex flex-wrap gap-4">
            <a href="/cv/CV_Rifqi_Ramadhan.pdf" download className="px-8 py-3 bg-cyan-500 text-black font-bold rounded-md shadow-lg shadow-cyan-500/40 hover:scale-105 transition-all">
              {t.download}
            </a>
            <a href="https://wa.me/6281357961978" target="_blank" className="px-8 py-3 border-2 border-cyan-500 text-cyan-500 font-bold rounded-md hover:bg-cyan-500 hover:text-black transition-all hover:scale-105">
              {t.talk}
            </a>
          </div>
        </div>
