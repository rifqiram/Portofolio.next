    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          float colorFactor = clamp(abs(uSpeed) * 15.0, 0.0, 1.0);
          vec3 finalRGB = mix(grayscale, color.rgb, colorFactor);
...
class App {
  constructor(container, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, dragOnly = false } = {}) {
  ...
  addEventListeners() {
    ...
    if (!this.dragOnly) window.addEventListener('wheel', this.boundOnWheel);

    // ✨ Pasang listener hanya ke container
  ...
  destroy() {
    ...
    if (!this.dragOnly) window.removeEventListener('wheel', this.boundOnWheel);
...
export default function CircularGallery({
  items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, dragOnly = true
}) {
  const containerRef = useRef(null);
  useEffect(() => {
    const app = new App(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, dragOnly });
    return () => app.destroy();
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, dragOnly]);
